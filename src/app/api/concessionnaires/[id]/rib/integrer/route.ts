...
import { integrateBancarisation, sanitizeRibWorkflowPublic } from "@/lib/lonaci/rib-bancarisation";

const bodySchema = z.object({
  compteBancaire: z.string().min(4).max(128),
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    const updated = await integrateBancarisation({
