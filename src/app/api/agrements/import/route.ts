...
const bodySchema = z
  .object({
    ...
    /** Produit cible : chaque agrément créé/mis à jour est rattaché à ce produit. */
...
function parseRowsFromJsonOrCsv(fileName: string, content: string): Record<string, unknown>[] {
  ...
  throw new Error(
    "Format fichier non supporté côté API (utilisez .json ou .csv, ou Excel côté navigateur).",
  );
}

export async function POST(request: NextRequest) {
  const rateLimitResponse = await enforceRateLimit(request, {
    namespace: "agrements-import",
    max: 20,
    windowMs: 10 * 60 * 1000,
  });
  ...
  if (!produitCode) {
    return badRequest(
      "Sélectionnez un produit avant d’importer.",
      "PRODUIT_REQUIRED",
    );
  }
  ...
  const summary = await importAgrementsFromRows(rows, auth.user, { produitCode });
  return NextResponse.json(
    {
      message: `Import terminé : chaque agrément a été rangé dans l’agence indiquée dans le fichier (produit ${produitCode}).`,
      produitCode,
